...
// ✅ تحويل الكمية لنطاق للتجار — الأدمن يشوف الرقم الحقيقي
function stockRange(stock: number): string {
  if (stock === 0) return 'نافد';
  const step = 5;
  const low  = Math.floor(stock / step) * step;
  const high = low + step;
  return `${low === 0 ? 1 : low} - ${high}`;
}

function applyStockDisplay(product: any, isAdmin: boolean): any {
  if (isAdmin) return product;
  return {
    ...product,
    stock:        stockRange(product.stock),
    stockDisplay: stockRange(product.stock),
    // نخفي السعر الحقيقي من الأدمن عن التجار
    companyWholesalePrice: undefined,
  };
}
...
const filtered = activeOnly ? all.filter((p: any) => p.isActive !== false) : all;

// ✅ تحقق من role — لو مو أدمن نحول الكمية لنطاق
const isAdmin = req.user?.role === 'admin';
const result  = filtered.map((p: any) => applyStockDisplay(p, isAdmin));
...
// ✅ نطاق للتجار، رقم حقيقي للأدمن
const isAdmin = req.user?.role === 'admin';
res.json(applyStockDisplay(p, isAdmin));
